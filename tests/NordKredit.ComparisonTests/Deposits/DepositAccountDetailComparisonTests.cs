...
/// <summary>
/// Comparison tests for deposit account detail inquiry — parallel-run validation against mainframe output.
/// COBOL source: CVACT01Y.cpy — ACCOUNT-RECORD (300 bytes), primary key lookup on ACCT-ID.
/// Business rules: DEP-BR-001 (data structure), DEP-BR-003 (balance tracking).
/// Regulations: FFFS 2014:5 Ch.3, Deposit Guarantee Directive 2014/49/EU, GDPR Art.15.
///
/// Golden file: Deposits/GoldenFiles/deposit-account-detail.json
...
/// - Status values: COBOL uses 'Y'/'N'. Migrated system uses Active/Dormant/Frozen/Closed.
///
/// Parallel-run stubs (not yet connected to mainframe):
/// - CompareDepositAccountDetail_Active_MatchesMainframeOutput
/// - CompareDepositAccountDetail_NotFound_MatchesMainframeOutput
/// - CompareDepositAccountDetail_Closed_MatchesMainframeOutput
/// </summary>
public class DepositAccountDetailComparisonTests
{
    private const string _goldenFilePath = "Deposits/GoldenFiles/deposit-account-detail.json";
    ...
    public void GoldenFile_ContainsExpectedFields()
    {
        ...
        Assert.True(root.TryGetProperty("productType", out _));
        ...
        Assert.True(root.TryGetProperty("holderName", out _));
        Assert.True(root.TryGetProperty("openedDate", out _));
        Assert.True(root.TryGetProperty("accruedInterest", out _));
    }
    ...
    public void GoldenFile_DateFormat_IsISO8601()
    {
        ...
        var openedDate = root.GetProperty("openedDate").GetString();
        Assert.NotNull(openedDate);
        Assert.Matches(@"^\d{4}-\d{2}-\d{2}$", openedDate);

        var postingDate = root.GetProperty("lastInterestPostingDate").GetString();
        Assert.NotNull(postingDate);
        Assert.Matches(@"^\d{4}-\d{2}-\d{2}$", postingDate);
    }
    ...
    [Fact]
    public void GoldenFile_AccruedInterest_Uses4DecimalPrecision()
    {
        var json = File.ReadAllText(_goldenFilePath);
        using var document = JsonDocument.Parse(json);
        var accrued = document.RootElement.GetProperty("accruedInterest").GetDecimal();

        Assert.Equal(accrued, decimal.Round(accrued, 4));
    }
}
