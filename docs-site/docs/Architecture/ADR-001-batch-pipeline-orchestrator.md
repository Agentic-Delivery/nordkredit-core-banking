---
title: "ADR-001: Batch Pipeline Orchestrator Pattern"
sidebar_label: "ADR-001: Batch Pipeline Orchestrator"
---
...
The nightly batch pipeline runs four sequential steps: card verification, credit/expiration validation, transaction posting, and report generation. These replace the JCL job chain (CBTRN01C &rarr; CBTRN02C &rarr; CBTRN03C). We need an orchestrator to coordinate the pipeline execution order, pass results between steps, handle errors, and monitor SLA compliance.
...
- Failed transactions must be filtered between steps (verified-only &rarr; valid-only).
...
2. Filters results between steps (only verified &rarr; only valid)
...
- COBOL source: JCL job chain CBTRN01C &rarr; CBTRN02C &rarr; CBTRN03C
- Regulations: FFFS 2014:5 Ch.4 &sect;3 (operational risk), DORA Art.11 (ICT risk management)
